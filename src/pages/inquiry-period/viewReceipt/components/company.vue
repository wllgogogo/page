                        ...
                        <div>
                            ...
                            <div class="rule_right1 collpan">
                                ...
                                <span style="display: inline-block;width: 320px;" v-show="formItem.billType == 'G'">
                                    <Input style="width:150px;vertical-align: middle;" :disabled="Disabled" type="number" v-model.number="formItem.billNumber" placeholder="请输入"> </Input>
                                ...
                                <Upload style="display: inline-block" :disabled="Disabled" :show-upload-list="false" :format="['jpg', 'jpeg', 'png']" :max-size="2048" :on-success="billSuccess" :on-exceeded-size="handleMaxSize" :on-error="handleErr" type="drag" :action="action">
...
export default {
    ...
    methods: {
        ...
        billSuccess({ data }) {
            // const url = `${urlPre}/${urlLink}/${data.group}/${data.filePath}`;
            console.log(data, '上传成功 对账单模板 响应')
            // this.formItem.billPath = ''
            const url = data.fileName + '~' + data.code
            this.formItem.billPath = url
        }
    }
    // watch:{}
    // defaultData: {
